...
export default async function InsightPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (item.lang === "en") notFound();
  return <main><Header path="/insights" /><article className="article shell"><header><Link className="back-link" href="/insights">← 返回洞察</Link><p className="eyebrow">{item.series || "洞察"} · {category?.zh}</p><h1>{item.title}</h1><p className="article-deck">{item.description}</p><div className="article-meta"><span>{item.date}</span><span>{languageLabel[item.lang]}</span><span>{item.tags.join(" · ")}</span></div></header><div className="article-layout"><aside><span>知识领域</span><strong>{category?.zh}</strong></aside><div className="article-body"><ArticleBody body={item.body} /></div></div></article><Footer /></main>;
}
